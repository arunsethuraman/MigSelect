...
void set_autoc_vals (double *pautoc_vals, int currentid); //AS: adding currentid 
...
//AS: adding currentid to this, so I can calculate autocorrelations for the coldchain only
void
set_autoc_vals (double *pautoc_vals, int currentid)
{
  ...
  if (lpgpd_v->do_autoc == 1)
  {
	...
	if ( z < 0 && currentid == 0) {
		pautocrec = 0.0;
		MPI_Recv(&pautocrec, 1, MPI_DOUBLE, MPI_ANY_SOURCE, 5656, MPI_COMM_WORLD, &status);
		pautoc_vals[i] = pautocrec;
		i++;
	}
	if (z >= 0 && currentid != 0) {
		pautocrec = C[z]->allpcalc.pdg + C[z]->allpcalc.probg;
		MPI_Send(&pautocrec, 1, MPI_DOUBLE, 0, 5656, MPI_COMM_WORLD);
		i++;
	}
	#endif
  ...
  for (j = 0; j < numsplittimes; j++)
  {
	...
	if (z < 0 && currentid == 0) {
		pautocrec = 0.0;
		MPI_Recv(&pautocrec, 1, MPI_DOUBLE, MPI_ANY_SOURCE, 6767, MPI_COMM_WORLD, &status);
		pautoc_vals[i] = pautocrec;
		i++;
	}
	if (z >= 0 && currentid != 0) {
		pautocrec = C[z]->tvals[j];
		MPI_Send(&pautocrec, 1, MPI_DOUBLE, 0, 6767, MPI_COMM_WORLD);
		i++;
	}
	#endif
  ...
  for (li = 0; li < nloci; li++)
  {
	...
	if ( z < 0 && currentid == 0) {
		pautocrec = 0.0;
		MPI_Recv(&pautocrec, 1, MPI_DOUBLE, MPI_ANY_SOURCE, 7878, MPI_COMM_WORLD, &status);
		pautoc_vals[i] = pautocrec;
		i++;
	}
	if (z >= 0 && currentid != 0) {
		pautocrec = C[z]->G[li].roottime;
		MPI_Send(&pautocrec, 1, MPI_DOUBLE, 0, 7878, MPI_COMM_WORLD);
		i++;
	}
	#endif
    }
...
init_autoc_pointers (void)
{
  ...
  if (autoc_pointer == NULL)
  {
    ...
    autoc_pointer = malloc (num_autoc * sizeof (struct autoc *));
    autoc_str_pointer = malloc (num_autoc * sizeof (char *));
    autoc_vals = malloc (num_autoc * sizeof (double));
    i = 0;
    if (lpgpd_v->do_autoc == 1)
    {
    ...
    if (assignmentoptions[POPULATIONASSIGNMENT] == 1)
    {
      ...
      autoc_a_pointer = malloc (num_autoc_a * sizeof (struct autoc *));
      autoc_a_vals = malloc (num_autoc_a * sizeof (double));
      autoc_a_str_pointer = malloc (num_autoc_a * sizeof (char *));
      i = 0;
      for (li = 0; li < nloci; li++)
        if (L[li].a_rec->v->do_autoc == 1)
        {
...
nextstepsave[AUTOCTERMS] for each lag value, the next step number at which a value is recorded 
...
checkautoc (int start_autocorrelations, int burndone, int burnsteps, int currentid)
{
  ...
  if (start_autocorrelations == 1)
  {
    ...
    for (i = 0; i < AUTOCTERMS; i++)
    {
      nextstepsave[i] = CHECKAUTOCWAIT + AUTOCINT * AUTOCSTEPSCALAR;
