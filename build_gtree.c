...
addA (int ci, int li, int newedge, int *nd0, int *nd1)
{
  ...
  for (; ai < L[li].nlinked; ai++)
  {
    ...
    p = malloc ((abs (gtree[up[0]].A[ai] - gtree[up[1]].A[ai]) + 3) * sizeof (double));
...
makeHKY (int ci, int li, int nosimmigration)
{
  ...
  struct edge *gtree = C[ci]->G[li].gtree;
  int count;
  edgevec = malloc ((2 * L[li].numgenes - 1) * sizeof (int));
  ...
  for (i = L[li].numgenes; i < 2 * L[li].numgenes - 1; i++)
  {
    gtree[i].hkyi.scalefactor = malloc (L[li].numsites * sizeof (double));
    gtree[i].hkyi.oldscalefactor = malloc (L[li].numsites * sizeof (double));
  ...
  ids = malloc ((L[li].numgenes) * (sizeof (int)));
  for (i = 0; i < L[li].numgenes; i++)
    ids[i] = i;
...
makeIS (int ci, int li, int nosimmigration)
{
  ...
  curid = malloc ((L[li].numgenes) * (sizeof (int)));
  singletons = malloc ((L[li].numsites) * (sizeof (int)));
  curgenes = L[li].numgenes;
