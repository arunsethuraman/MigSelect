...
fillplist (int ci)
{
  ...
  /* fill C[ci]->plist */
  if ((C[ci]->plist = malloc (npops * sizeof (*C[ci]->plist))) == NULL)
    IM_err (IMERR_MEM, "  plist malloc did not work.   npops %d, step %d",
            npops, step);
  for (i = 0; i < npops; i++)
  {
    if ((C[ci]->plist[i] =
         malloc ((npops - i) * sizeof (*C[ci]->plist[i]))) == NULL)
      IM_err (IMERR_MEM,
...
poptreeread (int ci, char *poptreestring)
{
  ...
  for (i = 0; i < npops; i++)
  {
    ...
    C[ci]->poptree[i].up = malloc (2 * sizeof (int));
    for (j = 0; j < 2; j++)
...
setup_poptree (int ci, char startpoptreestring[])
{
  ...
  if (npops == 1 )
  {
    C[ci]->poptree = malloc (sizeof (struct popedge));
    C[ci]->poptree[0].numup = 2;
    C[ci]->poptree[0].up = malloc (2 * sizeof (int));
  ...
  else if (assignmentoptions[POPULATIONASSIGNMENTINFINITE] == 1)
  {
    ...
    C[ci]->poptree = malloc ((npops + 1) * sizeof (struct popedge));
    C[ci]->poptree[npops].numup = npops;
    C[ci]->poptree[npops].up = malloc (npops * sizeof (int));
    C[ci]->poptree[npops].down = -1;
    ...
    C[ci]->plist = malloc (2 * sizeof (int *));
    C[ci]->plist[0] = malloc (npops * sizeof (int));
    C[ci]->plist[1] = malloc (sizeof (int));
    for (i = 0; i < npops; i++)
      C[ci]->plist[0][i] = i;
  ...
  else
  {
    ...
    C[ci]->poptree = malloc (numtreepops * sizeof (struct popedge));
