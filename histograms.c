...
// fillvec
// fills the xy vectors of iparams (used in printhistograms)
// this will go through a grid of values in the domain of the parameter
// and get the marginal distribution (y axis) for each point in x axis
void
fillvec (void)
{
  int i, j; // VS int i, j, p;
  int nbthetaparams, nbparams; // VS
  int gp=0; // VS

  nbthetaparams = nbgroupsloci_theta*numpopsizeparams; // VS
  nbparams = nbthetaparams + (nbgroupsloci_mig*nummigrateparams); // VS

  for (j = 0; j < GRIDSIZE; j++)
  {
    for (i = 0; i < nbthetaparams; i++) // VS for (i = 0, p = 0; i < numpopsizeparams; i++, p++)
    {
      itheta[g_paramindex[i]].xy[g_gp_ip[i]][j].y = margincalc ((double) itheta[g_paramindex[i]].xy[g_gp_ip[i]][j].x, 0.0, i, 0); // VS itheta[i].xy[gp][j].y = margincalc ((double) itheta[i].xy[gp][j].x, 0.0, p, 0);
    }
    for (i = nbthetaparams; i < nbparams; i++) // VS for (i = 0; i < nummigrateparams; i++, p++)
    {
      if (imig[g_paramindex[i]].pr[g_gp_ip[i]].max > MPRIORMIN) // VS if (imig[i].pr[gp].max > MPRIORMIN)
      {
        imig[g_paramindex[i]].xy[g_gp_ip[i]][j].y = margincalc ((double) imig[g_paramindex[i]].xy[g_gp_ip[i]][j].x, 0.0, i, 0); //  VS imig[i].xy[gp][j].y = margincalc ((double) imig[i].xy[gp][j].x, 0.0, p, 0);
      }
    }
  }
}                               /* fillvec */
...
writehistogram (FILE * outfile, int numhistprint, int dosmooth)
{
   ...
   * before being used.
   */ 
  smthprobvals = calloc ((size_t) numhistprint, sizeof (double));
...
void prepare_parameter_histograms (int *numhistprint)
{
  ...
  // VS
  int nbthetaparams, nbparams;
  nbthetaparams = nbgroupsloci_theta*numpopsizeparams; // VS
  nbparams = nbthetaparams +(nbgroupsloci_mig*nummigrateparams); // VS

  fillvec (); // VS function that fills the iparam.xy structure with the marginal distribution
  *numhistprint = 0;

  for (i = 0; i < nbthetaparams; i++) // VS for (i = 0; i < numpopsizeparams; i++)
  {
    hp[*numhistprint].xy = itheta[g_paramindex[i]].xy[g_gp_ip[i]]; // VS hp[*numhistprint].xy = itheta[i].xy[gp];
    strcpy (hp[*numhistprint].str, itheta[g_paramindex[i]].str); // VS strcpy (hp[*numhistprint].str, itheta[i].str);
  ...
  for (i = nbthetaparams; i < nbparams; i++) // VS for (i = 0; i < nummigrateparams; i++)
    if (imig[g_paramindex[i]].pr[g_gp_ip[i]].max > MPRIORMIN) // VS if (imig[i].pr[gp].max > MPRIORMIN)
    {
      hp[*numhistprint].xy = imig[g_paramindex[i]].xy[g_gp_ip[i]]; // VS hp[*numhistprint].xy = imig[i].xy[gp];
      strcpy (hp[*numhistprint].str, imig[g_paramindex[i]].str); // VS strcpy (hp[*numhistprint].str, imig[i].str);
      hp[*numhistprint].yscaleadjust = 1;
      hp[*numhistprint].xscaleadjust = 1;
      hp[*numhistprint].before = 0;
      hp[*numhistprint].after = 0;
...
{
  int i;
  int nbthetaparams; //  VS nbthetaparams
  ...
  for (i = 0; i < nbthetaparams; i++) // VS for (i = 0; i < numpopsizeparams; i++)
  { 
    hp[*numhistprint].xy = itheta[g_paramindex[i]].xy[g_gp_ip[i]]; // VS hp[*numhistprint].xy = itheta[i].xy[gp]; // VS gp
    strcpy (hp[*numhistprint].str, itheta[g_paramindex[i]].str); // VS strcpy (hp[*numhistprint].str, itheta[i].str);
    hp[*numhistprint].yscaleadjust = 1;
                                               ...
                                               int prob_or_like)
{
  ...
  if (modeloptions[PARAMETERSBYPERIOD])
  {
    for (k = 0; k < lastperiodnumber; k++)
    {
      for (i = 0; i < npops - k; i++)
      {
        mpop = C[0]->plist[k][i]; //leaving this as it is, since plist shouldn't change AS - Tue Feb 23 15:56:18 EST 2016
        ...
        for (mi = 0; mi < nummigrateparams; mi++)
        {
          ...
          if (found)
          {
            ...
            {
              pmmax = EXPOMIGPLOTSCALE * imig[mi].pr[gp_mig].mean; // VS gp_mig
            }
            else
            {
              pmmax = itheta[thetai].pr[gp_theta].max * imig[mi].pr[gp_mig].max / 2.0; // VS gp
            }
            j = GRIDSIZE-1;
            maxxfind = 0;
            while (maxxfind==0 && j >= 0)
            {
              ...
              if (modeloptions[EXPOMIGRATIONPRIOR])
                tempy = calc_pop_expomig (thetai, mi,tempx , prob_or_like, gp_mig, gp_theta); // VS gp
              else
                tempy = calc_popmig (thetai, mi,tempx , prob_or_like, gp_mig, gp_theta); // VS gp
              if (tempy > 1e-9)
                maxxfind = tempx;
              else 
                j--;
            ...
            for (j = 0; j < GRIDSIZE; j++)
            {
              ...
              {
                popmigxy[hpi][j].y = calc_popmig (thetai, mi, popmigxy[hpi][j].x, prob_or_like, gp_mig, gp_theta);
              }
            }
    ...
    for (i = 0; i < numtreepops - 1; i++)
    {
      ...
      for (mi = 0; mi < nummigrateparams; mi++)
      {
        ...
        if (found)
        {
          ...
          if (modeloptions[EXPOMIGRATIONPRIOR])
          {
            pmmax = EXPOMIGPLOTSCALE * imig[mi].pr[gp_mig].mean; // VS gp
          }
          else
          {
            pmmax = itheta[thetai].pr[gp_theta].max * imig[mi].pr[gp_mig].max / 2.0; // VS gp
          ...
          for (j = 0; j < GRIDSIZE; j++)
          {
            popmigxy[hpi][j].x = (j + 0.5) * maxxfind / GRIDSIZE;
            if (modeloptions[EXPOMIGRATIONPRIOR])
              popmigxy[hpi][j].y = calc_pop_expomig (thetai, mi, popmigxy[hpi][j].x, prob_or_like, gp_mig, gp_theta); // VS gp
            else
              popmigxy[hpi][j].y = calc_popmig (thetai, mi, popmigxy[hpi][j].x, prob_or_like, gp_mig, gp_theta); // VS gp
          }
          hp[hpi].xy = popmigxy[hpi];
          hp[hpi].xscaleadjust = hp[hpi].yscaleadjust = 1;
          hp[hpi].before = 0;
                      ...
                      double generationtime, double scaleumeaninput)
{
  ...
  // VS Are we multiplyng by 2 the nummigrateparamas to take into account the 2NM histograms?
  // VS If so, for the groups of loci, we may need to re-think a little bit this because we start to have a large number of histograms
  predictparamhist = (nbgroupsloci_theta*numpopsizeparams) + (2 * nbgroupsloci_mig*nummigrateparams); // VS included nbgroupsloci_theta and nbgroupsloci_mig
...
void printmigrationhistograms (FILE * outfile, long int recordstep)
{
  ...
  if (nloci > 1)
  {
    ...
    FP "    All migration events recorded in the coalescent direction (i.e. backwards in time)\n");
