...
#define RELEASE_DATE "October 13, 2010"

//AS: MPI functionality check throughout the code
#define MPI_ENABLED
...
#ifndef _MSC_VER
#ifndef __forceinlineVS
#define __forceinlineVS __attribute__((__always_inline__)) inline
#endif /*  */
#endif /*  */
...
// VS defined the following constants
#define MAXGROUPS 500 // VS define the maximum number of groups allowed
#define MAXPARAMS 500 // VS define the maximum number of parameters allowed (this is used to get the size of gextern int g_gp_gs) 
...
/* VS
 * Assignment of loci related options 
 * Usage:
 *   -al0: do not update assignment of loci vectors
 *   -al1: update assignment of loci vector a_mig
 *   -al2: update assignment of loci vector a_theta
 *   -al3: jointly update assignment of loci vectors a=a_mig=a_theta
 *   -al4: perform the likelihood ratio test looking at theta and mig params independently for two populations 
 * lociassignmentoptions[LLR];
 */
enum
{
  NOTUPDATEASSIGLOCI  = 0,
  UPDATEASSIGNMIG     = 1,
  UPDATEASSIGNTHETA   = 2,
  UPDATEASSIGNMIGTHETA= 3,
  LRTTWOPOP           = 4,
};
...
struct i_param
{
  struct priorvalues *pr; // Array of size nbgroups				
  struct plotpoint **xy; // Two dimensional array of size NB_GROUPS (rows) and nbpoints (columns). If used points to an array of gridize elements, each a plotpoint
  int *indexgroup; // Array of size nbloci with the group index of each locus, 
  // e.g. 3 loci and 2 groups (0,0,1) means loci 0 and 1 belong to group 0 and locus 2 to  group 1

  // Constant variables that do not depend on number of groups of loci
  strn str;	// Name of the parameter
...
struct chain
{
  ...
  // VS
  // this will have the sum of weights for the groups to which each locus belongs to
  // Based on the gweights matrices of each locus, we can constructu the gweight for the group
  // as the sum of the weights for the loci that belong to a given group.
  // The question here is that we will need grouploci_weights for theta params (cc, fc) and 
  // other gweights for mig params (mc, fm).
  // This means that to compute the P(G) we need to compute the integration over the parameter
  // values independently for theta and migration.
  struct genealogy_weights* groupgweight_theta;  // accumulated stuff used to calculate p(G) for theta groups of loci
  struct probcalc* grouppcalc_theta;     // prob(Data|G) and prob(G) stuff for theta groups of loci
  
  struct genealogy_weights* groupgweight_mig;  // accumulated stuff used to calculate p(G) for mig groups of loci
  struct probcalc* grouppcalc_mig;     // prob(Data|G) and prob(G) stuff for mig groups
...
// VS
// global files to look at gweights
gextern FILE *fgweights_loci;
gextern FILE *fgweights_all;
gextern FILE *fgweights_theta;
gextern FILE *fgweights_mig;
// VS
// global files to look at pcalc
gextern FILE *fpcalc_all;
gextern FILE *fpcalc_theta;
gextern FILE *fpcalc_mig;

// VS 10/11/2011 included this variable to only look for the migration parameter
// even when looking only at the case with two populations
gextern int vs_test; 
...
//AS: adding numprocesses and currentid as globals
gextern int numprocesses;
//gextern int currentid;
gextern int nloci;
gextern int npops;
...
// VS - options related with assignment of loci and LRT 
gextern int assignmentlocioptions[LRTTWOPOP+1];
...
//AS: adding an array allbetas, which saves all the temperatures for printing
...
// VS external variables to save the assignment vectors of loci into groups
gextern int **assignlocisample_mig;
gextern int **assignlocisample_theta;
...
gextern struct extendnum *eexpsum; // VS - what is this variable????
...
// VS
gextern int nbgroupsloci_theta; // nb of groups of loci that share the same theta parameters
gextern int nbgroupsloci_mig; // nb of groups of loci that share same mig parameters

// VS
gextern double *reltheta; // relative theta - array of size nloci
gextern double *relmig; // relative migration - array of size nloci

// VS
// The user define which loci belong to each group for theta and mig parameters
// this is an array of size nbloci - each element points to the group index of a given loci
gextern int **grouploci_theta; // size nloci, array with 0,1,0,0 where each element is the group of loci for each locus
gextern int **grouploci_mig; // size nloci


//AS
//Adding swapcounting stuff - yet to add functions for these in the code s on 9/26/2014
...
//Adding swapcounting based on temperature stuff - on Mon Aug 17 12:01:02 EDT 2015
...
// VS 
// global variable that records the acceptance rate of assignment vector
// TO BE DONE: put these values into a struct chainstate_record_updates_and_values?
//gextern int assignmigtries; // number of attempts to update assignment of loci
//gextern int assignmigacc; // number of times the assignment vector is updated
// VS 5/18/2012
struct chainstate_record_updates_and_values *assignloci;
...
void checkautoc (int start_autocorrelations, int burndone, int burnsteps, int currentid); //AS: adding currentid
...
// VS added this function to compute the group gweights when updating a single locus
void sum_subtract_treeinfo_vs (struct genealogy_weights *addup,
							struct genealogy_weights *addtoplus,
							struct genealogy_weights *addtominus, int theta_or_mig);

// VS added this function to compute the group gweights when updating the assignment of loci
...
//AS: adding coldchain ID to savegsampinf
void savegsampinf (float *g, int z);
// VS - function to save the gsampinf for each group of loci sharing same param
void savegsampinf_vs (float *g, int gp, int theta_or_mig, int z); //AS: adding z - coldchain ID
// VS - function to save the assignment vectors of loci into groups
...
// VS - sum only cc, hc and fc weights
void sum_treeinfo_theta_vs (struct genealogy_weights *addup,
							struct genealogy_weights *addto);
// VS - sum only mc, and fm weights
void sum_treeinfo_mig_vs (struct genealogy_weights *addup,
						  struct genealogy_weights *addto);
// VS - check the gweights
void print_gweight_vs (struct genealogy_weights *check, int theta_or_mig);
void print_gweight_vs_file (struct genealogy_weights *check, int theta_or_mig, FILE *file);
...
void printrunbasics (FILE * outfile, int loadrun, char fpstr[],
                     ...
                     double hilike, double hiprob/*, int step*/, int z); //AS: adding z (cold chain id) Tue Feb 23 15:50:19 EST 2016
...
// VS - savegenealogyfile_vs - gets the group index as input
void savegenealogyfile_vs (int gp, char genealogyinfosavefilename[], FILE * genealogyinfosavefile,
                   int lastgenealogysavedvalue, int gsampinflength);

// VS save the assignment vectors to groups of loci into a file
void saveassignlocifile(char genealogyinfosavefilename[], int lastgenealogysavedvalue, int **sampledassign);
...
// double margincalceexp (double x, double yadust, int pi, int logi); not used as of 8/25/09
...
// VS changed this function to
//void mnbrakmod (int ndim, int paramindex, int gp, int firsttree, int lasttree, double *ax, double *bx,
//                double *cx, double *fa, double *fb, double *fc,
//                double (*func) (int, int, int, int, int, double, int), int ifneeded);
void mnbrakmod (int ndim, int firsttree, int lasttree, double *ax, double *bx,
                double *cx, double *fa, double *fb, double *fc,
                double (*func) (int, int, int, double, int), int ifneeded);
...
//                  double (*func) (int, int, int, double, int), int ifneeded);
// VS changed this function to - in the end went back to the same as before because created global variables for the indexes
double goldenmod (int ndim, int firsttree, int lasttree, double ax,
                  double bx, double cx, double tol_, double *xmin,
                  double (*func) (int, int, int, double, int), int ifneeded);

// VS - for the functions to find 2NM, the previous version of goldenmod 
// did not work. This is because we need to specify the index for the groups of theta and mig
// The function marginalopt_popmig calls goldenmod, but with the wrong gp indexes and func
// Thus, need to create a function specific for the case when there are several groups of loci.
double goldenmod_2nm (int ndim, int gp_mig, int gp_theta, int firsttree, int lasttree, double ax,
                  double bx, double cx, double tol_, double *xmin,
                  double (*func) (int, int, int, int, int, double, int), int ifneeded);
...
// VS modified these functions such that they get as input the gp_mig and gp_theta indexes
...
// VS functions
double calc_popmig (int thetai, int mi, double x, int prob_or_like, int gp_mig, int gp_theta); // VS gp
double calc_pop_expomig (int thetai, int mi, double x, int prob_or_like, int gp_mig, int gp_theta);// VS gp



void marginalopt_popmig (int firsttree, int lasttree, double *mlval, double *peakloc, int *mpop, int *mterm);
// VS changed this function to include as input the gp_mig and gp_theta
// double marginpopmig (int mi, int firsttree, int lasttree, double x, int thetai);
double marginpopmig (int mi, int gp_mig, int gp_theta, int firsttree, int lasttree, double x, int thetai);

/**** GLOBAL FUNCTIONS IN gtint.c ****/
void print_greater_than_tests (FILE * outfile);

/**** GLOBAL FUNCTIONS IN swapchains.c ****/
void setheat (double hval1, double hval2, int heatmode, int currentid); //AS: adding currentid

/* CR 110929.4 get rid of extraneous args in declaration  */
int swapchains (int swaptries,int swapbetasonly, int currentid); //AS: adding currentid
void swapbetas (int ci, int cj); //AS: adding a function to swap just the temperatures, not the whole chain pointers

//AS: adding a function for swapping heats between temperatures
#ifdef MPI_ENABLED
...
void printchaininfo (FILE * outto, int heatmode, 
                     double hval1, double hval2, int currentid); //AS: adding currentid
...
// VS 3/11/2011
// new realloc functions that check if the realloc was successful
...
// VS - function to integrate tree probability with li in input
void initialize_integrate_tree_prob_vs (int ci, struct genealogy_weights *gweight,
										struct probcalc *pcalc, int gp, 
										int theta_or_mig_or_both);

// VS - function to check that qintegrate for mig groups is set to zero, and that
// mintegrate for theta groups is set to zero.		
void check_pcalc_groups_vs(struct probcalc *check, int theta_or_mig);
// VS - function to print the pcalc structures to the output
void print_pcalc_groups_vs(struct probcalc *check);
void print_pcalc_groups_vs_file(struct probcalc *check, FILE *pcalcfile);

// VS - function to check that qintegrate for mig groups is set to zero, and that
// mintegrate for theta groups is set to zero.		
void check_pcalc_groups_vs(struct probcalc *check, int theta_or_mig);
// check the gweights for groups of loci for theta or migration parameters
void check_gweight_vs (struct genealogy_weights *check, int theta_or_mig);
...
// VS - function created to integrate tree for each locus
void integrate_tree_prob_vs(int ci,
                     struct genealogy_weights *gweight,
                     struct genealogy_weights *holdgweight,
                     struct probcalc *pcalc,
                     struct probcalc *holdpcalc,
				     double *holdt,
					 int gp, int theta_or_mig);
...
// VS added the function to update the loci assignment
...
int changet_RY1 (int ci, int p/*, double tv*/); //AS: added tv as C[z]->tvals[lastperiodnumber]
//AS: edited upon discussion with Jody Tue Feb 23 13:50:41 EST 2016
...
// VS 3/11/2011
// new realloc functions that check if the realloc was succesfull or not
...
void setup_multi_t_arrays (int z); //AS: adding currentid
...
// VS GLOBAL FUNCTIONS ADDED BY VS
// These have the index and are used in functions histograms.c, surface_search_functions.c and jointfind.c
// i			= param 		: 0<i<(nbgroupsloci_theta*numposizeparams)+(nbgroupsloci_mig*nummigrateparams)
// paramindex 	= index param in iparam	: 0<paramindex<numposizeparams for theta
//										: 0<paramindex<nummigrateparams for mig
// gp_iparam	= index group in iparam	: 0<gp_iparam<nbgroupsloci_theta for theta
//										: 0<gp_iparam<nbgroupsloci_mig for mig
// gp_gsampinf	= index group gsampinf	: 0<gp_gsampinf<nbgroupsloci_theta+nbgroupsloci_mig
gextern int g_paramindex[MAXGROUPS]; // index of index of parameters in iparam structures
gextern int g_gp_ip[MAXGROUPS]; // index of group gp in the context of iparam structures
gextern int g_gp_gs[MAXGROUPS*MAXPARAMS]; // index of group gp in the context of gsampinf structure

//These variables are used to record the index of the parameters.
//The order of the parameters depends on the structure we are looking at.
//These variables are a kind of lookup table with the corresponding index depending on the context.
//For instance, in a model with 2 groups of loci for theta and 2 groups of loci for migration (same loci share the same loci), consider that the assignment vector is ztheta=(0,0,1,1) and for zmig=(0,0,1,1).
//The order of the parameters, if we take into account all parameters is:
//q0_g0 q1_g0 qA_g0 q0_g1 q1_g1 qA_g1 m01_g0 m10_g0 m01_g1 m10_g1 (0<i<(nbgroupsloci_theta*numposizeparams)+(nbgroupsloci_mig*nummigrateparams)) 
//
//If we look into the iparam structures, the index of the parameters depends on whether we are looking at the paramindex, or group index.
//For imig[g_paramindex], g_paramindex goes from 0 to nummigrateparams and corresponds to: m01 and m10
//For imig[0].prior[gp_iparam], gp_iparam goes from 0 to nbgroupsloci and corresponds to: m01_g0 m01_g1 
//
//If we look into the gsampinf 3D-matrix, each 2D-matrix corresponds to the gweights of a certain group, and the first index corresponds to the group of theta and mig put together.
//For the gsampinf[gp_gsampinf] gpsampinf goes from 0 to nbgroupsloci_theta+nbgroupsloci_mig anc corresponds to: Theta_g0 Theta_g1 Mig_g0 Mig_g1
//
//i 		= param 		: 0<i<(nbgroupsloci_theta*numposizeparams)+(nbgroupsloci_mig*nummigrateparams)
//paramindex 	= index param in iparam	: 0<paramindex<numposizeparams for theta
//					: 0<paramindex<nummigrateparams for mig
