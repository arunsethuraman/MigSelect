...
aread (FILE * mcffile, const char *name, int atype, int iucheck, void *a)
{
  ...
  {
  case 0:
    for (i = 0, ip = a; i < iu; i++, ip++)
      fscanf (mcffile, "%d ", ip);
    break;
  case 1:
    for (i = 0, lip = a; i < iu; i++, lip++)
      fscanf (mcffile, "%ld ", lip);
    break;
  case 2:
    for (i = 0, fp = a; i < iu; i++, fp++)
      fscanf (mcffile, "%g ", fp);
    break;
  case 3:
    for (i = 0, dp = a; i < iu; i++, dp++)
      fscanf (mcffile, "%lg ", dp);
    break;
  case 4:
    for (i = 0, cp = a; i < iu; i++, cp++)
      fscanf (mcffile, "%c", cp);
    *cp = '\0';
    break;
  case 5:
    for (i = 0, ulp = a; i < iu; i++, ulp++)
