...
//extern struct edgemiginfo  sumedge,sumsis;  //debug
...
/*int checkpop(int ci, int period, int pop)//8_30_10
{
  int found = 0;
  int pops;
  int i;
  pops = npops - period;
  i = 0;
  while (found == 0 && i< pops)
  {
    found = (pop == C[ci]->plist[period][i]) ;
    i++;
  }
  return found;
} */

/*int compareptype()//8_30_10
{
  int i, match;
  match = 1;
  i = 0;
  while (match == 1 && i< 5)
  {
    match = (checkptype[0][i]==ptype[0][i] && checkptype[1][i] == ptype[1][i]);
    match = match &&  (checkrr[0][i]==rr[0][i] && checkrr[1][i] == rr[1][i]);
    i++;
  }
  return match;
} */
...
{                               /* note fc includes inheritance scalar, i.e. it is fc/h  - see treeweight  
  ...
  if (cc > 0)
  {
	...
	//printf("fc here is %f\n", fc);
	//if (fc <= 0) {
	//	printf ("Hmmm...fc here is %f\n",fc);
	//}
    //AS debug only!!!!!assert (fc > 0);
    if (min == 0)
    {
          ...
          double timein, double upt, int pop, int constrainpop)
{
  ...
  assert (numm > 0);
  startm = lastm + 1;
  lastm = lastm + numm; // VS What is last m and start m?? The problem seems to be related with this.
  
  // VS CHECK
  // printf("Chain= %i Startm= %i Lastm= %i\n", ci, startm, lastm);
  ...
  if (numm > 1)
    ...
    hpsortmig (&edgem->mig[startm] - 1, numm);
       ...
       */
      for (migIndex = startm; migIndex < lastm; ++migIndex) // VS I think this is where the problem is. We need to sort the array from the beggining and not only startm and lastm
      {
...
init_gtreecommon (void)         // initialize copy edge
{
  ...
  //IMA_initmemory_edgemiginfo (&sumedge); //debug
  /*sumedge.dnt = sumedge.upt = 0;
  sumedge.fpop = 0;
  sumedge.b = sumedge.e = 0;
  sumedge.mpall = sumedge.mtall = 0;
  for (i = 0; i<=2; i++)
  {
    sumedge.mtimeavail[i] = 0;
    for (j=0;j<10;j++)
      sumedge.mig[j].mt  = 0;
  }*/
  //IMA_initmemory_edgemiginfo (&sumsis);   //debug
  /*sumsis.dnt = sumsis.upt = 0;
  sumsis.fpop = 0;
  sumsis.b = sumsis.e = 0;
  sumsis.mpall = sumsis.mtall = 0;
  for (i = 0; i<=2; i++)
  {
    sumsis.mtimeavail[i] = 0;
    for (j=0;j<10;j++)
    {
      sumsis.mig[j].mt  = 0;
      sumsis.mig[j].mp  = 0;
...
//same[i][j] has the number of combinations when there are i+3 species and j+2 mutations
//i.e.   for n species in the period, and m mutations on the edge  use same[n-3][m-2]
//good for up to 5 species and 9 mutations 
...
/*if (checkpp)
{
  for (periodi=0;periodi<5;periodi++)
  {
    ptype[0][periodi] = ptype[1][periodi] = -100;
    rr[0][periodi] = rr[1][periodi] = 0;
  }
} // debug  */
...
/*rr[0][periodi] = r;  //8_30_10
ptype[0][periodi] = -1;  //8_30_10 */
...
/*assert(checkpop(ci,edgem->e, pop[0])); //8_30_10 */
        topop = edgem->fpop;
/*assert(checkpop(ci,edgem->e, topop)); //8_30_10 */
        popc = (npops - edgem->e - 1);
        if (pop[0] == topop)
        {
            d = log (1 - r * exp (-r));
/*ptype[0][edgem->e] = 3;  //8_30_10 */
        }
        else           
        {
          d = log ((1 -  exp (-r)));
/*ptype[0][edgem->e] = 2;  //8_30_10 */
        }
...
/*assert(checkpop(ci,edgem->e, lastm_2_pop)); */
                  ...
                  n = edgem->mp[edgem->e] * log(r/edgem->mtimeavail[edgem->e]) - r + (2- edgem->mp[edgem->e]) * log(popc) + pathc;
                }
        }
        tempp += n - d;
      }
...
/*rr[ii][periodi] = r; //8_30_10
ptype[ii][periodi] = -1;  //8_30_10 */
...
/*if (checkpp)
  assert(rr[ii][mm->e] == 0); //8_30_10
rr[ii][mm->e] = r; //8_30_10 */
          /* if assignmentoptions[POPULATIONASSIGNMENTINFINITE] == 1  do not enter here */
...
/*ptype[ii][mm->e] = 0;  //8_30_10 */
          ...
          else                      // 3 or more pops in the last period 
          {
            ...
            {
              d = log (1 - r * exp (-r));
/*ptype[ii][mm->e] = 3;  //8_30_10 */
            }
            else           
            {
              d = log ((1 -  exp (-r)));
/*ptype[ii][mm->e] = 2;  //8_30_10 */
            }
            switch (mm->mp[mm->e])
            {
            ...
            default:
                    ...
                    if (lastm_2_pop == topop)
                      ...
                      pathc = -log((double) popc-1); 
                    n = mm->mp[mm->e] * log(r/mm->mtimeavail[mm->e]) - r + (2- mm->mp[mm->e]) * log(popc) + pathc;
                    break;
              }
            tempp += n - d;
          }
...
/*if (checkpp)
//if (edgem->mtall > 0 && sisem->mtall > 0 && edgem->e < lastperiodnumber)
    if (compareptype() == 0)   // debug
      IM_err (IMERR_ASN, "compareptype in step %d", step);
*/
...
//mwork_single_edge (int ci, struct edgemiginfo *edgem,struct edgemiginfo *oldedgem, int lastmigperiod,int ei)//debug 8/30/10
{
  ...
  for (periodi = edgem->b;periodi <= lastmigperiod; periodi++)
  {
    ...
    r = calcmrate(oldedgem->mp[periodi],oldedgem->mtimeavail[periodi])*edgem->mtimeavail[periodi]; //9/2/10
/*checkrr[ei][periodi] = r;  //8_30_10 */
    if (periodi < edgem->e)           // can do migration to any available pop
    {
/*checkptype[ei][periodi] = -1;  //8_30_10 */
    ...
    else
    {
      if (npops - periodi == 2)
      {
        ...
        {
/*checkptype[ei][periodi] = 0;//8_30_10 */
  ...
  for (periodi = b; periodi <= lastperiodi; periodi++)
  {
    ...
    for (ii=0;ii<2;ii++)
    {
      ...
      if (mm->b <= periodi)
      {
        ...
        r = calcmrate(oldmm->mp[periodi],oldmm->mtimeavail[periodi])*mm->mtimeavail[periodi];//9/2/10
/*checkrr[ii][periodi] = r; //8_30_10 */
/*checkptype[ii][periodi] = -1; */
...
/*pp *= MIGCLOSEFRAC; //debug 8_30_10 */
      }
      else
      {
        edgem->fpop = sisem->fpop = picktopop(edgem->temppop,C[ci]->plist[periodi],npops - periodi);
/*pp*= (1.0 - MIGCLOSEFRAC)/(npops - periodi-1); //debug 8_30_10 */
      }
    }
    else
    {
...
/*pp *= 0.5; //debug 8_30_10 */
      ...
      else
      {
        if (uniform() < MIGCLOSEFRAC) 
        {
/*pp *= (MIGCLOSEFRAC * 0.5); //debug 8_30_10 */
          if (uniform() < 0.5) 
            edgem->fpop = sisem->fpop = edgem->temppop;
          else
            edgem->fpop = sisem->fpop = sisem->temppop;
        ...
        {
/*pp *= (1.0 - MIGCLOSEFRAC) /(npops - periodi-2);  //debug 8_30_10 */
...
/*checkptype[ii][periodi] = 0;  //8_30_10 */
        }
        else                    // odd
        {
          mm->mp[periodi] = poisson (r, 1);
/*checkptype[ii][periodi] = 1;  //8_30_10 */
        }
        if (mm->mp[periodi] > 0)
          lastm[ii] =
...
/*checkptype[ii][periodi] = 3;  //8_30_10 */
        }
        else                    // cannot be zero migration events 
        {
          mm->mp[periodi] = poisson (r, 2);
/*checkptype[ii][periodi] = 2;  //8_30_10 */
        }
        if (mm->mp[periodi] > 0)
          lastm[ii] =
            simmpath (ci, mm, periodi, mm->mp[periodi], lastm[ii],
...
init_treeweight (void)
{
  ...
  sgEvent = malloc (elength * sizeof (struct gtreeevent));
  sgEvent_indx = malloc (elength * sizeof (unsigned long));
}                               // init_treeweight
...
*/
{
 ...
 G->mignum = 0;
  ...
  {
    sgEvent = realloc (sgEvent, (ncount + EADDLENGTH) * sizeof (struct gtreeevent));
    assert (sgEvent !=  NULL);
    sgEvent_indx = realloc (sgEvent_indx, (ncount + EADDLENGTH) * sizeof (unsigned long));
	// VS 3/11/2011
    //		sgEvent = realloc (sgEvent, (ncount + EADDLENGTH) * sizeof (struct gtreeevent));
    //		sgEvent_indx = realloc (sgEvent_indx, (ncount + EADDLENGTH) * sizeof (unsigned long));
	// with this function we are sure that the new array will have enough memory
    // because ncount is the number of events that we need to allocate
	sgEvent = reallocSgv(sgEvent, (ncount + EADDLENGTH));
	// VS 3/11/2011
	// with this function we are sure that the new array will have enough memory
	// because ncount is the number of events we need to allocate
	sgEvent_indx = reallocSgI(sgEvent_indx, (ncount + EADDLENGTH));
  ...
  for (i = 0; i < 2 * ng - 1; i++)
  {
    ...
    while (gtree[i].mig[j].mt > -0.5)
    {
      ...
      assert (ISELEMENT (nowpop, C[ci]->periodset[sgp->periodi])); //AS: changed after talking to Jody Tue Feb 23 15:27:46 EST 2016
      sgp->pop = nowpop;
  ...
  for (j = 0; j < ec; j++)
  {
	...
	//printf("gweight->fc is %f\n ", gweight->fc[k][ii]);
      assert(gweight->fc[k][ii] >= 0);//AS: should this be > 0 not >= 0?
                     ...
                     struct probcalc *holdpcalc, double *holdt)
{
  ...
  else
  {
    ...
    for (i = 0; i < numpopsizeparams; i++)
    {
      ...
      {
        pcalc->qintegrate[i] =
          integrate_coalescent_term (c, f, hc, itheta[i].pr[gp].max, // VS gp
                                     itheta[i].pr[gp].min); // VS gp
		//AS: debug only
		//if (f <= 0) {
			//printf("Hmmm.f is coming from here...1\n");
		//}
      }
      ...
      for (i = 0; i < nummigrateparams; i++)
      {
        ...
        else
        {
          ...
          if (modeloptions[EXPOMIGRATIONPRIOR])
            pcalc->mintegrate[i] =
              integrate_migration_term_expo_prior (c, f, imig[i].pr[gp].mean); // VS gp
          else
            pcalc->mintegrate[i] =
              integrate_migration_term (c, f, imig[i].pr[gp].max, imig[i].pr[gp].min);  // VS gp
        }
...
// INTEGRATE_TREE_PROB_VS 
// integrate the gweights for theta or mig parameters only
// INPUT:
//	int ci : index of chain
//	genealogy_weights *gweight : pointer to structure with gweights of new G'
//	genealogy_weights *holdgweight : pointer to structure with gweights of old G
//	probcalc *pcalc : pointer to structure where the pcalc will be saved for G'
//	probcalc *holdpcalc : pointer to structure where the pcalc for G are stored
//	int gp : index for group of loci
//	int theta_or_mig : 1 if THETA (compute qintegrate)
//					   0 if MIG (compute mintegrate)
// PRE-REQUISITES
//	all gweight and pcalc structures need to be initialized
...
{
  //int vm_aux;   // VS auxiliary variable to print check
  double psum; // this will have the sum of the integral terms - P(G*)
  int i, j;
  int c, holdc;
  double hc, f, holdf;
  int checkm;
  ...
  else
  {
	...
	// if locus belongs to group theta
	if(theta_or_mig==1) { 

		// VS
		// This for goes from pop 1 to pop n
		// numpopsizeparams refers to the number of parameters refering to the population size 4Nu_j
		// If there are 3 sampled populations, there are 6 parameters 4Nu
		// Note that the i_param refers to a structure that has information on the prior of theta
		for (i = 0; i < numpopsizeparams; i++)
		{
		  c = holdc = 0;
		  f = holdf = hc = 0.0;
		  
		  // wp values indicate what gweights are needed to integrate a given param
		  // e.g. for itheta[0] we need all gweights for periods where pop0 existed
		  // wp.n refers to the nb of terms used to calculate the weight
		  for (j = 0; j < itheta[i].wp.n; j++)
		  {	
			// weight position wp of a given parameter (e.g.) itheta refers to the terms needed to compute the integration
			c += gweight->cc[itheta[i].wp.p[j]][itheta[i].wp.r[j]];
			holdc += holdgweight->cc[itheta[i].wp.p[j]][itheta[i].wp.r[j]];
			f += gweight->fc[itheta[i].wp.p[j]][itheta[i].wp.r[j]];
			//AS: debug only
			//printf("f being calculated here is %f\n",f);
			holdf += holdgweight->fc[itheta[i].wp.p[j]][itheta[i].wp.r[j]];
			hc += gweight->hcc[itheta[i].wp.p[j]][itheta[i].wp.r[j]];
		  }
		  tempc += c;
		  ...
		  {
			  // VS
			  // integrate coalescent terms over the THETA params
			  // If each group of locus has its own theta prior, we need to give it here as input to this function
			  //AS debug
			 // printf ("f term %f \n", f);
			  pcalc->qintegrate[i] = integrate_coalescent_term(c, f, hc, itheta[i].pr[gp].max, itheta[i].pr[gp].min);
			//AS: debug only
			//if (f <= 0) {
			//	printf("Hmmm...f is coming from here...2\n");
			//}
		  }
	...
	//assert(tempc == total_numgenes-nloci); // VS ? when we compute this considering allgweights this assert must be true
	assert(holdtempc==tempc);
	...
	if(theta_or_mig==0) { 
		...
		// Integrating over the migration parameters
		if (!modeloptions[NOMIGRATION])
		{

		  // VS - nummigrateparams is the length of the array imig[i], composed of strcutures i_params
		  // The imig array has numigrateparams elements that refers to the pairs of migration
		  // It is independent of the period in which both populations appear, i.e each pair only appears once
		  for (i = 0; i < nummigrateparams; i++)
		  {
			c = holdc = 0;
			f = holdf = 0.0;
			...
			{
			  c += gweight->mc[imig[i].wp.p[j]][imig[i].wp.r[j]][imig[i].wp.c[j]];
			  holdc += holdgweight->mc[imig[i].wp.p[j]][imig[i].wp.r[j]][imig[i].wp.c[j]];
			  f += gweight->fm[imig[i].wp.p[j]][imig[i].wp.r[j]][imig[i].wp.c[j]];
			  holdf += holdgweight->fm[imig[i].wp.p[j]][imig[i].wp.r[j]][imig[i].wp.c[j]];
			}
			if (c == holdc && f == holdf)
			{
			  pcalc->mintegrate[i] = holdpcalc->mintegrate[i];
			...
			{
				if (modeloptions[EXPOMIGRATIONPRIOR]) {
					// VS pcalc->mintegrate[i] = integrate_migration_term_expo_prior (c, f, imig[i].pr.mean);
					pcalc->mintegrate[i] = integrate_migration_term_expo_prior (c, f, imig[i].pr[gp].mean); 
				}
				else {
					// VS pcalc->mintegrate[i] = integrate_migration_term (c, f, imig[i].pr.max, imig[i].pr.min);
					pcalc->mintegrate[i] = integrate_migration_term (c, f, imig[i].pr[gp].max, imig[i].pr[gp].min); 
				}
			}
			psum += pcalc->mintegrate[i];
		  }
...
printf("Here1???\n");
  ...
  else
  {
	...
	printf("Inside else2\n");
	printf("itheta... = %d", itheta[i].pr[gp].max);
	printf("itheta...2 = %d", itheta[i].pr[gp].min);
      pcalc->qintegrate[i] =
        integrate_coalescent_term (c, f, hc, itheta[i].pr[gp].max,  // VS gp
                                   itheta[i].pr[gp].min);  // VS gp
	...
	//AS: debug only
	//if (f <=0 ){
	//	printf("Hmmm...f is coming from here...3\n");
	//}
      psum += pcalc->qintegrate[i];
	printf("Inside else 4\n");
      assert (psum > -1e200 && psum < 1e200);
    }
	printf("Here??2\n");
    if (!modeloptions[NOMIGRATION])
...
// VS
// INITIALIZE_INTEGRATE_TREE_PROB_VS
// same as integrate_tree_prob but with index of loci as input to initialize the pcalc of each genealogy
// same as integrate_tree_prob but does not have checks to see if values match current ones */
// computes the integral of gweights over the prior for theta and migration parameters
// INPUT:
//   int ci : index for chain
//	 struct genealogy_weights *gweight : gweights matrix
//	 struct probcalc *pcalc : pcalc object where the results of integrals will be saved
//	 int gp : group of loci (may refer to theta or mig)
//	 int theta_or_mig_or_both : 1 if THETA, 0 if MIG, 2 if BOTH integrals should be computed
// PRE-REQUISITES
//	gweight must have been initialized before
void initialize_integrate_tree_prob_vs (int ci, struct genealogy_weights *gweight,
									    struct probcalc *pcalc, int gp, int theta_or_mig_or_both)
{
  double psum;
  int i, j;
  int c;
  ...
  // initialize all the qintegrate and mintegrate to zero
  for (i = 0; i < numpopsizeparams; i++) {
	  pcalc->qintegrate[i] = 0;
  }
  for (i = 0; i < nummigrateparams; i++) { // VS 8/23/2011 found a bug here - the index should be nummigrateparams and not numpopsizeparams
  ...
  else
  {
	...
	// Get the sum of the gweights for the parameter of interest
	// This assumes that we already have the gweights for the groups of loci computed correctly
	if(theta_or_mig_or_both==1 || theta_or_mig_or_both==2) {
		...
		for (i = 0; i < numpopsizeparams; i++)
		{
		  ...
		  // VS -  pcalc->qintegrate[i] = integrate_coalescent_term (c, f, hc, itheta[i].pr.max,itheta[i].pr.min);
		  pcalc->qintegrate[i] = integrate_coalescent_term (c, f, hc, itheta[i].pr[gp].max,itheta[i].pr[gp].min);
			//AS: debug only
			//if(f <=0) {
			//	printf("Hmmm...f is coming from here..4\n");
			//}
		  psum += pcalc->qintegrate[i];
		  assert (psum > -1e200 && psum < 1e200);
		}
	}
	if(theta_or_mig_or_both==0 || theta_or_mig_or_both==2) {
		if (!modeloptions[NOMIGRATION])
		{
		  for (i = 0; i < nummigrateparams; i++)
		  {
			...
			if (modeloptions[EXPOMIGRATIONPRIOR])
			{
			  pcalc->mintegrate[i] = integrate_migration_term_expo_prior (c, f, imig[i].pr[gp].mean); // VS - pcalc->mintegrate[i] = integrate_migration_term_expo_prior (c, f, imig[i].pr.mean);
			}
			else
			{
			  pcalc->mintegrate[i] = integrate_migration_term (c, f, imig[i].pr[gp].max, imig[i].pr[gp].min); // VS -  pcalc->mintegrate[i] = integrate_migration_term (c, f, imig[i].pr.max, imig[i].pr.min);
			}
			psum += pcalc->mintegrate[i];
		  }
		}
...
// VS
// CHECK_PCALC_GROUPS_VS
// adds the pcalc to addup. Useful to add the log(Integral) of each group to allpcalc
// and obtain the prior probability of the genealogy taking into account all groups
// INPUT:
//	int ci : chain index
//	probcalc *addup : pointer to probcalc to which addto will be added (usually allpcalc)
//	probcalc *addto : pointer to probcalc to add to addto (usually pcalc from a given group)
...
void check_pcalc_groups_vs(struct probcalc *check, int theta_or_mig)
{
	...
	// Check that probcalc addto are zero for mintegrate is theta and vice-versa
	if(theta_or_mig==1) { // theta group
		for(i=0; i<nummigrateparams; i++) {
			sum += check->mintegrate[i];
		}
		if(sum>0) {
			printf("ERROR: mintegrate of grouptheta was larger than zero for mig!");
			exit(1);
		}
	}
	else { // mig group
		for(i=0; i<numpopsizeparams; i++) {
			sum += check->qintegrate[i];
		}
		if(sum>0) {
			printf("ERROR: qintegrate of groupmig was larger than zero for theta!");
			exit(1);
		}
...
// VS
// PRINT_PCALC_GROUPS_VS
// print pcalc structures to check the values
// INPUT:
//	probcalc *check : pointer to probcalc to which addto will be added (usually allpcalc)
//	int theta_or_mig : 1 if THETA group, 0 if MIG group
void print_pcalc_groups_vs(struct probcalc *check)
{
	...
	printf("\nCheck Qintegrate");
	for(i=0; i<numpopsizeparams; i++) {
		printf("\nqintegrate[%i]=%g", i, check->qintegrate[i]);
	}
	printf("\nCheck Mintegrate");
	for(i=0; i<nummigrateparams; i++) {
		printf("\nmintegrate[%i]=%g", i, check->mintegrate[i]);
	}
	printf("\npdg=%g", check->pdg);
	printf("\nprobg=%g", check->probg);
...
void print_pcalc_groups_vs_file(struct probcalc *check, FILE *pcalcfile)
{
	...
	//printf("\nCheck Qintegrate");
	for(i=0; i<numpopsizeparams; i++) {
		fprintf(pcalcfile, "%g ", check->qintegrate[i]);
	}
	//printf("\nCheck Mintegrate");
	for(i=0; i<nummigrateparams; i++) {
		fprintf(pcalcfile, "%g ", check->mintegrate[i]);
	}
	fprintf(pcalcfile, "pdg= %g ", check->pdg);
	fprintf(pcalcfile, "probg= %g ", check->probg);
...
getnewt (int timeperiod, double t_u_prior, double t_d_prior, double oldt, int whichupdate)
{
  ...
  //debug  no windowing     return t_u_prior + uniform()*(t_d_prior - t_u_prior);

  twin =  (t_d_prior - t_u_prior) / (log (nloci + 1) * (npops - timeperiod));
  U = uniform ();
...
static void *reallocSgv(struct gtreeevent *sge, int n) {
	...
	struct gtreeevent *newSge = realloc(sge, n*sizeof(struct gtreeevent));
...
static void *reallocSgI(unsigned long *sgeI, int n) {
	...
	// create a new pointer that will save the realloc of the old one
	unsigned long *newSgeI = realloc(sgeI, n*sizeof(unsigned long));
