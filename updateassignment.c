                      ...
                      struct edgemiginfo *newe)
{
  ...
  //newe->mpall = mwork_single_edge (ci, newe, newe->e, mparamf);  8_30_10
      ...
      if (gk->thetas[pc3] == 0.0)
        {
          ...
          (*rates)[2] = 2 * (2 - 1) / itheta[pc3].pr[gp].max;  // VS gp
        }
      else
        {
...
imaGsampinfPermutedG (int gi, float *atree, int *od)
{
  ...
  for (pi = 0; pi < npops; pi++)
    {
      pj = od[pi];
      atree[gsamp_ccp + pi] = gsampinf[gp][gi][gsamp_ccp + pj]; // VS gp
      atree[gsamp_fcp + pi] = gsampinf[gp][gi][gsamp_fcp + pj]; // VS gp
      atree[gsamp_hccp + pi] = gsampinf[gp][gi][gsamp_hccp + pj]; // VS gp
      atree[gsamp_qip + pi] = gsampinf[gp][gi][gsamp_qip + pj]; // VS gp
    }
      ...
      for (pj = pi + 1; pj < npops; pj++)
        {
          ...
          atree[gsamp_mcp + i] = gsampinf[gp][gi][gsamp_mcp + j]; // VS gp
          atree[gsamp_fmp + i] = gsampinf[gp][gi][gsamp_fmp + j]; // VS gp
          atree[gsamp_mip + i] = gsampinf[gp][gi][gsamp_mip + j]; // VS gp
...
imaGsampinfRelabel (int gi, int *od)
{
  ...
  memcpy (gsampinf[gp][gi], atree, gsampinflength * sizeof (float)); // VS gp
  XFREE (atree);
