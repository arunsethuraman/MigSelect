...
// Details
//	the variable gtree->cmm stores the size of the migration array
//  the function will check if n>gtree->cmm. If so, it reallocates memory to size n.
__forceinlineVS void
...
// VS
// This function gets as input a double (in log) and returns the exponential of that
// It returns the values in scientific notation exp(x)=m*10^z
// INPUT:
//	double x : log of the variable for which we want to get the exponential
//	double *m : pointer to the term multiplied by exp(x)=m*10^z
//  double *z : pointer to the power of the base 10
